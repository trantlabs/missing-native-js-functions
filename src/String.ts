...
define(String.prototype, {
    ...
    similarity: function (second: string) {
        ...
        for (let i = 0; i < first.length - 1; i++) {
            ...
            const countLow = lowBigrams.has(bigram.toLowerCase())
                ? lowBigrams.get(bigram.toLowerCase()) + 1
                : 1
        ...
        for (let i = 0; i < second.length - 1; i++) {
            ...
            const countLow = firstBigrams.has(bigram.toLowerCase())
                ? firstBigrams.get(bigram.toLowerCase())
                : 0
            if (count > 0) {
